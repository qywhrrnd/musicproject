...
public class MusicCrawlingScheduler {
    ...
    @Scheduled(cron = "0 0/47 15 * * *")
    public void rnbCrawling() throws IOException {

        int i = 1;
        for (int page = 1; page < 3; page++) {
            String url = "https://music.naver.com/listen/genre/top100.nhn?domain=OVERSEA_V2&genre=P04&duration=1d&page=" + page;
            Document doc = Jsoup.connect(url).get();
    ...
    @Scheduled(cron = "0 0/59 2 * * *")
    public void jpopCrawling() throws IOException {

        int i = 1;
        for (int page = 1; page < 3; page++) {
            String url = "https://music.naver.com/listen/genre/top100.nhn?domain=OVERSEA&genre=P06&page=" + page;
            Document doc = Jsoup.connect(url).get();
